...
class Medoo
{
	...
	public function select_context($table, $join, &$columns = null, $where = null, $column_fn = null)
    {
    ...
    //获取statement，方便fetch设置不同方式。
    public function selectStm($table, $join, $columns = null, $where = null)
    {
        return $this->query($this->select_context($table, $join, $columns, $where));
    }

    //结果集中，以某一列为key
    public function selectResultGroup($table, $join, $columns = null, $where = null)
    {
        $stm = $this->query($this->select_context($table, $join, $columns, $where));

        return $stm ? $stm->fetchAll(PDO::FETCH_GROUP|PDO::FETCH_UNIQUE|PDO::FETCH_ASSOC) : false;
    }

    //两列中第一列变成key
    public function selectKeyPair($table, $join, $columns = null, $where = null)
    {
        $stm = $this->query($this->select_context($table, $join, $columns, $where));

        return $stm ? $stm->fetchAll(PDO::FETCH_KEY_PAIR) : false;
    }
			...
			foreach ($data as $key => $value)
			{
				...
				switch (gettype($value))
				{
					case 'NULL':
						$values[] = 'NULL';
						break;
                    #TODO:数组的话会打成json or 序列化对象，感觉没毛用
					case 'array':

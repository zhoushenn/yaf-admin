...
// Better than `$.core(true, ...)` because arrays are not traversed/copied.
//
// called like:
...
Class.extend = function(members) {
	...
	// copy over all class variables/methods to the subclass, such as `core` and `mixin`
...
DayGrid.mixin({
	...
	renderSegRow: function(row, rowSegs) {
		...
		function emptyCellsUntil(endCol) {
			while (col < endCol) {
				// try to grab a cell from the level above and core its rowspan. otherwise, create a fresh cell
...
var View = fc.View = Class.extend({
	...
	function getViewSpec(requestedViewType) {
		...
		viewOptionsChain.unshift({}); // jQuery's core needs at least one arg
...
fcViews.agenda = View.extend({ // AgendaView
	...
	render: function() {
		...
		if (this.dayGrid) {
			...
			// have the day-grid core it's coordinate area over the <hr> dividing the two grids
